...
async def handle_job_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # FIX Issue 6: HTML Injection Escaped for every field!
        def render_tier_jobs(jobs_list, fallback_city):
            ...
            for j in jobs_list[:2]:
                ...
                link_html = f'   🔗 <a href="{safe_link}">Click here to Apply directly</a>\n' if safe_link else ""

                res.append(
                    f"   🏢 <b>Company:</b> <u>{c_name}</u>\n"
                    f"   📍 <b>Location:</b> {c_loc}\n"
                    f"   💼 <b>Post:</b> {c_title}\n"
                    f"   💰 <b>Expected Salary:</b> {html.escape(intel['estimated_salary'])}\n"
                    f"   ⏱️ <b>Hiring Status:</b> {html.escape(intel['posting_freshness'])}\n"
                    f"   🥊 <b>Competition:</b> {html.escape(intel['competition_level'])}\n"
                    f"   ⭐ <b>Company Rating:</b> {html.escape(intel['company_rating'])}\n"
                    f"   👻 <b>Ghost Job Risk:</b> {html.escape(intel['ghost_risk'])}\n"
                    f"   💡 <b>Tip:</b> <i>{html.escape(intel['time_saver_tip'])}</i>\n"
                    f"{link_html}"
                )
            return "\n".join(res)

        fresher_skills = html.escape(', '.join(fresher_spec.get('skills', [])))
        fresher_tools = html.escape(', '.join(fresher_spec.get('tools', [])))
        mid_skills = html.escape(', '.join(mid_spec.get('skills', [])))
        mid_tools = html.escape(', '.join(mid_spec.get('tools', [])))
        senior_skills = html.escape(', '.join(senior_spec.get('skills', [])))
        senior_tools = html.escape(', '.join(senior_spec.get('tools', [])))

        # Format Tiered HTML Report with fully escaped variables
        report = (
            ...
            f"💰 <b>Salary Range:</b> {html.escape(fresher_spec.get('salary', '₹3.5L - ₹5.5L LPA'))}\n"
            f"🛠 <b>Skills:</b> {fresher_skills}\n"
            f"🧰 <b>Tools:</b> {fresher_tools}\n"
            f"🏢 <b>Hiring Companies & Locations:</b>\n"
            f"{render_tier_jobs(fresher_jobs, location)}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"🟡 <b>2. MID-LEVEL SPECIALIST (2 - 4 Yrs Exp)</b>\n"
            f"💰 <b>Salary Range:</b> {html.escape(mid_spec.get('salary', '₹7.0L - ₹13.0L LPA'))}\n"
            f"🛠 <b>Skills:</b> {mid_skills}\n"
            f"🧰 <b>Tools:</b> {mid_tools}\n"
            f"🏢 <b>Hiring Companies & Locations:</b>\n"
            f"{render_tier_jobs(mid_jobs, location)}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"🔴 <b>3. SENIOR / LEAD (5+ Yrs Exp)</b>\n"
            f"💰 <b>Salary Range:</b> {html.escape(senior_spec.get('salary', '₹15.0L - ₹25.0L+ LPA'))}\n"
            f"🛠 <b>Skills:</b> {senior_skills}\n"
            f"🧰 <b>Tools:</b> {senior_tools}\n"
            f"🏢 <b>Hiring Companies & Locations:</b>\n"
